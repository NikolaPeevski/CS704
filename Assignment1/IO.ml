...
let rec print_db e = 
  let rec print_part = 
    ( function
      | (D.Var i, _, _) ->
          if !need_space then print_space (); print_int i; need_space := true
      | (D.Lambda e, Left, _) ->
          l_paren(); print_lambda e; r_paren();
      | (D.Lambda(e), Right, Left) ->
          l_paren(); print_lambda e; r_paren()
      | (D.Lambda(e), _, _) ->
          print_lambda e;
  ...
  and print_lambda e =
    print_string "\\"; need_space := false; print_cut (); 
      open_box 0; print_part (e, Down, Down); close_box()
...
let lam_of_string (s:string) = 
  ...
  LambdaParse.term LambdaLex.tokenize lexbuf

let db_of_string (s:string) =
  let lexbuf = Lexing.from_string s in
  DeBruijnParse.term DeBruijnLex.tokenize lexbuf
